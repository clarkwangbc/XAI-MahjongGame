    ...
    { // 通知全部玩家开始游戏
        ...
        { // 机器人作弊用，用于分析其他玩家的牌
            ...
            for (uint8_t j = 0; j < GAME_PLAYER; j++)
            {
                if (j == i)
                    continue;
                auto uint8_num = GameLogic::switchToCardData(m_cbCardIndex[j], &GameStart.cbCardData[MAX_COUNT * bIndex++], MAX_COUNT);
                auto temp = m_cbCardIndex[j];
            }
        }
        IGameEngineEventListener *pListener = m_pIPlayer[i]->getGameEngineEventListener();
        if (pListener != NULL)
...
bool GameEngine::onUserOutCard(CMD_C_OutCard OutCard)
{
    ...
    if (!GameLogic::removeCard(m_cbCardIndex[m_cbCurrentUser], OutCard.cbCardData))
    { //删除扑克
        log("removeCard fail");
        return true;
    }
...
bool GameEngine::dispatchCardData(uint8_t cbCurrentUser, bool bTail)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < m_cbWeaveItemCount[i]; j++) {
			if (m_WeaveItemArray[i][j].cbWeaveKind == WIK_P) {
				for (int k = 0; k < 3; k++) {
					auto temp = to_string(GameLogic::switchToCardIndex(m_WeaveItemArray[i][j].cbCenterCard)) + ",";
					discard_str.append(temp);
				}
					
			}
			else if (m_WeaveItemArray[i][j].cbWeaveKind == WIK_G) {
				for (int k = 0; k < 4; k++) {
					auto temp = to_string(GameLogic::switchToCardIndex(m_WeaveItemArray[i][j].cbCenterCard)) + ",";
					discard_str.append(temp);
				}
	...
	if (cbCurrentUser == 0)
	{
		...
		if (!out_file.is_open()) {
			...
			out_file << player_cards_str << endl;
			out_file.close();
		}

		//写入玩家的听牌表
		ofstream player_ting_out_file;
		player_ting_out_file.open("C:/Users/clark/MahjongGame/XAIMethod-AutoLevel/ResultLogs/PlayerTingLog.txt");
		if (!player_ting_out_file.is_open()) {
			log("ERROR: OPEN FILE FAILED");
		}
		else {
			player_ting_out_file << player_ting_str << endl;
			player_ting_out_file.close();
		}
    ...
    if (m_cbUserAction[cbCurrentUser] != WIK_NULL)
    { //存在暗杠、拐弯杠、或者自摸
    ...
    { //动作判断
        ...
        if (estimateKind == EstimateKind_OutCard) //出牌方式为普通出牌
        {
            ...
            if (m_cbLeftCardCount > 0) //有剩余的牌才能杠
            {
                ...
                if ((m_cbUserAction[i] & WIK_G) != 0x00)
                { //有杠
                    m_cbGangCount = 1;
...
bool GameEngine::onUserOperateCard(CMD_C_OperateCard OperateCard)
{
    ...
    if ((cbChairID != m_cbCurrentUser) && (m_cbCurrentUser != INVALID_CHAIR))
    {
        return true;
    }                                                //前面判断自己摸的牌，后面判断玩家打的牌
    memset(m_llHuRight, 0, sizeof(m_llHuRight));     //清空胡牌类型
    ...
    {
        if (m_bResponse[cbChairID])
        {
            return true;
        } //判断是否重复处理，防止多次提交
        if ((cbOperateCode != WIK_NULL) && ((m_cbUserAction[cbChairID] & cbOperateCode) == 0x00))
        {
            return true;
        }                                       //操作状态不对
    ...
    if (m_cbCurrentUser == cbChairID) //当前用户为操作用户
    {
        ...
        if (!GameLogic::isValidCard(cbOperateCard))
        {
            return true;
        };                                             //判断牌是否有效
        ...
        case WIK_G: //杠牌操作
        {
            ...
            if (GangCardResult.cbPublic[cbGangIndex] == TRUE)
            {                                                                     //明杠
                ...
                for (uint8_t i = 0; i < m_cbWeaveItemCount[m_cbCurrentUser]; i++) //遍历组合总数，设置牌
                {
                    ...
                    if ((cbCenterCard == cbOperateCard) && (cbWeaveKind == WIK_P))            //如果组合为碰
                    {
                        ...
                        if (m_cbSendCardData != cbOperateCard)
                        {                                                     //发的牌和杠的牌不一致，则不算分
            ...
            if (GangCardResult.cbPublic[cbGangIndex] == FALSE)
            {                                                                                    //暗杠
            ...
            if (GangCardResult.cbPublic[cbGangIndex] == TRUE)
                bAroseAction = estimateUserRespond(cbChairID, cbOperateCard, EstimateKind_GangCard); //枪杠判定
            if (!bAroseAction)                                                                       //不存在枪杠情况
            {
                m_bQiangGangStatus = false;        //不枪杠
                dispatchCardData(cbChairID, true); //杠发牌
...
bool GameEngine::onEventGameConclude(uint8_t cbChairID)
{
    ...
    { //计算杠分
        ...
        {
            if (m_WeaveItemArray[i][j].cbWeaveKind == WIK_G)
            { //没被抢的杠、和后杠
                if (m_WeaveItemArray[i][j].cbPublicCard == TRUE && m_WeaveItemArray[i][j].cbProvideUser != i)
                { //放的杠2番
                ...
                else if (m_WeaveItemArray[i][j].cbPublicCard == FALSE && m_WeaveItemArray[i][j].cbProvideUser == i)
                { //暗杠
